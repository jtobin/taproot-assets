...
CREATE TABLE authmailbox_messages (
...
CREATE TABLE reorg_anchorings (
    ...
    -- sensed phase is witnessed or buried. Indexed so that a new
    -- registration can find the live anchoring whose witness created
    -- its trigger outpoints (automatic dependency-edge derivation).
    witness_txid BLOB
...
CREATE INDEX reorg_anchorings_witness_txid_idx
    ON reorg_anchorings(witness_txid);
...
CREATE TABLE tx_proof_claimed_outpoints (
