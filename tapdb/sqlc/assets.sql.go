...
SELECT seedling_id, asset_name, asset_version, asset_type, asset_supply, asset_meta_id, emission_enabled, batch_id, group_genesis_id, group_anchor_id, script_key_id, group_internal_key_id, group_tapscript_root, delegation_key_id, universe_commitments
...
type MarkManagedUTXOAsSweptParams struct {
	Outpoint     []byte
	SweepingTxid []byte
}

func (q *Queries) MarkManagedUTXOAsSwept(ctx context.Context, arg MarkManagedUTXOAsSweptParams) error {
	_, err := q.db.ExecContext(ctx, MarkManagedUTXOAsSwept, arg.Outpoint, arg.SweepingTxid)
	return err
}
...
func (q *Queries) QueryAssetBalancesByAsset(ctx context.Context, arg QueryAssetBalancesByAssetParams) ([]QueryAssetBalancesByAssetRow, error) {
	query := QueryAssetBalancesByAsset
	var queryParams []interface{}
