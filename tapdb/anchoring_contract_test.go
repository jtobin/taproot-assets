...
type contractWorld struct {
	...
	// foreclosures are the transactions the chain decided for, when
	// the abandonment has a known cause. Between them they consume a
	// drawn subset of inputPoints, which compensation must then leave
	// spent. Empty models an abandonment without a cause to bound the
	// reversal.
	foreclosures []*wire.MsgTx
...
func genContractWorld(rt *rapid.T, t *testing.T, f *contractFixture,
	...
	// The foreclosing transactions, when the loss has a cause: one
	// or two of them share a drawn subset of the transfer's inputs,
	// holding the compensation to the union of their input sets
	// rather than to any single one.
	if numInputs > 0 && rapid.Bool().Draw(rt, "foreclosure") {
		numForeclosers := rapid.IntRange(1, 2).Draw(rt, "foreclosers")
		for i := 0; i < numForeclosers; i++ {
			pkScript := bytes.Repeat([]byte{0xef + byte(i)}, 34)
			w.foreclosures = append(w.foreclosures, &wire.MsgTx{
				TxOut: []*wire.TxOut{{
					PkScript: pkScript,
					Value:    500,
				}},
		...
		for i, dbID := range w.inputDBIDs {
			...
			forecloser := rapid.IntRange(0, numForeclosers-1).Draw(
				rt, fmt.Sprintf("forecloser%d", i),
			)
			w.foreclosures[forecloser].TxIn = append(
				w.foreclosures[forecloser].TxIn, &wire.TxIn{
