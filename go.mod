...
// This branch depends on one not-yet-upstream lnd change:
//
//   ChannelReservation.AuxChanState() — populates LocalChanCfg /
//   RemoteChanCfg from the negotiated contributions before
//   DescFromPendingChanID fires.
//
// The replace points at the local lnd worktree so the in-tree change can
// be tested together. Replace with an upstream tag once it is merged.
