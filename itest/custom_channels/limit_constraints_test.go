...
"description":"USD stablecoin for limit constraint test"
	...
	// Make sure Erin knows about the Charlie → Dave channel.
	assertChannelKnown(t.t, erin, chanPointCD)
	...
	const invoiceAssetAmount = 10_000_000
	invoiceResp := createAssetInvoice(
		t.t, dave, erin, invoiceAssetAmount, assetID,
	)
	...
	numUnits, _ := payInvoiceWithAssets(
		t.t, charlie, dave, invoiceResp.PaymentRequest,
	...
	// Pay using the FOK quote.
	invoiceRespFOK := createAssetInvoice(
		t.t, dave, erin, invoiceAssetAmount, assetID,
	)
	...
	numUnitsFOK, _ := payInvoiceWithAssets(
		t.t, charlie, dave, invoiceRespFOK.PaymentRequest,
	...
	// Pay using the IOC quote.
	invoiceRespIOC := createAssetInvoice(
		t.t, dave, erin, invoiceAssetAmount, assetID,
	)
